...
crew_id와 nickname을 컬럼으로 가지고, crew_id가 PK인 테이블로 만든다.
...
- 만약 crew 테이블에는 crew_id가 12번인 크루가 존재하지 않지만, attendance 테이블에는 여전히 crew_id가 12번인 크루가 존재한다면?
  - 해당 크루가 중간에 퇴소했거나
  - 누군가의 실수에 의해 레코드가 삭제되었거나

attendance.crew_id가 crew.crew_id를 참조하도록 설정한다.
